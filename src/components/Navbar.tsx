...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 border-b-4 border-black brutalist-bg transition-all duration-300`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            <Link to="/" className="flex items-center">
              <span className="text-2xl font-extrabold text-black uppercase tracking-widest">Frostwood Labs</span>
            </Link>
          </div>
      ...
      <div className={`md:hidden ${isOpen ? 'block' : 'hidden'}`}> 
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 brutalist-bg border-t-4 border-black">
          {navItems.map((item) => (
            <Link
              key={item.label}
              to={item.href}
